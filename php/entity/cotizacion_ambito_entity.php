...
// DolarYMonedas SDK CotizacionAmbito entity
...
class CotizacionAmbitoEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new CotizacionAmbitoEntity($this->_client, $opts);
    }

    /**
     * @param CotizacionAmbito|array $args CotizacionAmbito data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = DolarYMonedasHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return CotizacionAmbito|array The current CotizacionAmbito data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of CotizacionAmbito fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = DolarYMonedasHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of CotizacionAmbito fields).
     */
     ...
     *
     * @param CotizacionAmbitoLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed CotizacionAmbitoLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return CotizacionAmbito|array The loaded CotizacionAmbito as an assoc-array at the
     *   SDK boundary; throws DolarYMonedasError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = DolarYMonedasHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     *
     * @param CotizacionAmbitoListMatch|array|null $reqmatch Match filter (any subset
     *   of CotizacionAmbito fields) as an assoc-array; CotizacionAmbitoListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return CotizacionAmbito[]|array A list of CotizacionAmbito items as assoc-arrays at
     *   the SDK boundary; throws DolarYMonedasError on failure (item-5 convention).
     */

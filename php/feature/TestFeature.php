...
// DolarYMonedas SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class DolarYMonedasTestFeature extends DolarYMonedasBaseFeature
{
    ...
    public function init(DolarYMonedasContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (DolarYMonedasContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = DolarYMonedasParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (DolarYMonedasContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(DolarYMonedasContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = DolarYMonedasParam::call($ctx, $k);

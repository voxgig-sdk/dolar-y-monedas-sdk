...
class DolarYMonedasMakeError
{
    public static function call(?DolarYMonedasContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new DolarYMonedasContext([], null);
        }
        $op = $ctx->op ?? new DolarYMonedasOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new DolarYMonedasResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof DolarYMonedasError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new DolarYMonedasError('', $msg, $ctx);
        ...
        if ($err instanceof DolarYMonedasError) {

...
// DolarYMonedas SDK feature test
//
...
class FtCtrl extends DolarYMonedasControl
{
...
class FtClient
{
    ...
    private DolarYMonedasUtility $utility;

    public function __construct(DolarYMonedasUtility $utility, array $options)
    {
    ...
    public function get_utility(): DolarYMonedasUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (DolarYMonedasContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public DolarYMonedasContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (DolarYMonedasContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = DolarYMonedasConfig::make_feature($name);
    ...
    private static function buildUrl(DolarYMonedasSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof DolarYMonedasError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new DolarYMonedasResponse($fetched);
        }
    ...
    private function fail(DolarYMonedasContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (DolarYMonedasContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

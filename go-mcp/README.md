...
[MCP](https://modelcontextprotocol.io) server exposing the DolarYMonedas SDK as
two agent tools — `dolar-y-monedas_list` and `dolar-y-monedas_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/dolar-y-monedas-mcp)
make build

# 2. Provide credentials via the environment
export DOLAR_Y_MONEDAS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user dolar-y-monedas \
  -- /absolute/path/to/dolar-y-monedas-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./dolar-y-monedas-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// dolar-y-monedas_list: first page of records
{ "entity": "cotizacion_ambito" }
{ "entity": "cotizacion_ambito", "query": { } }

// dolar-y-monedas_load: one record by id
{ "entity": "blue", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/dolar-y-monedas-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user dolar-y-monedas \
     -- "$PWD"/dist/*/dolar-y-monedas-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `dolar-y-monedas_list` and `dolar-y-monedas_load` tools now appear
   in new sessions. Ask the agent to *"list cotizacion_ambito using dolar-y-monedas"*
   and it calls `dolar-y-monedas_list` with `{"entity":"cotizacion_ambito"}`.
...
```sh
export DOLAR_Y_MONEDAS_APIKEY=sk_live_xxx            # API key
export DOLAR_Y_MONEDAS_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `dolar-y-monedas_list` tool
...
### Call the `dolar-y-monedas_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `dolar-y-monedas_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `dolar-y-monedas_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `DOLAR_Y_MONEDAS_APIKEY` | API key sent with every request. |
| `DOLAR_Y_MONEDAS_BASE` | Optional override of the API base URL. |
...
```sh
./dolar-y-monedas-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"dolar-y-monedas_load","arguments":{"entity":"blue","query":{"id":1}}}}'
```

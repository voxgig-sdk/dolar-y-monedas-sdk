...
class CotizacioneEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return CotizacioneEntity(self._client, opts)
    ...
    def match_get(self) -> Cotizacione:
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Cotizacione]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Cotizacione().list() works with no args.
        if reqmatch is None:
            reqmatch = {}

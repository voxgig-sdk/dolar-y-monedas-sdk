...
from dolarymonedas_sdk.dolarymonedas_types import (
    CotizacionAmbito,
...
class CotizacionAmbitoEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return CotizacionAmbitoEntity(self._client, opts)
    ...
    def data_get(self) -> CotizacionAmbito:
    ...
    def match_get(self) -> CotizacionAmbito:
    ...
    def load(self, reqmatch=None, ctrl=None) -> CotizacionAmbito:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.CotizacionAmbito().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[CotizacionAmbito]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.CotizacionAmbito().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",

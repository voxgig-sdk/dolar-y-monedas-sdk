...
class TestCotizacioneDirect:

    def test_should_direct_list_cotizacione(self):
        setup = _cotizacione_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-cotizacione", "live" if setup["live"] else "unit")
        if _skip:
...
def _cotizacione_direct_setup(mockres):

...
class TestMayoristaDirect:

    def test_should_direct_load_mayorista(self):
        setup = _mayorista_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-mayorista", "live" if setup["live"] else "unit")
        if _skip:
...
def _mayorista_direct_setup(mockres):

...
class TestCotizacionAmbitoEntity:

    def test_should_create_instance(self):
        testsdk = DolarYMonedasSDK.test(None, None)
        ent = testsdk.CotizacionAmbito(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.CotizacionAmbito(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.CotizacionAmbito(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _cotizacion_ambito_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "cotizacion_ambito." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set DOLAR_Y_MONEDAS_TEST_COTIZACION_AMBITO_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        cotizacion_ambito_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.cotizacion_ambito")))
        cotizacion_ambito_ref01_data = None
        if len(cotizacion_ambito_ref01_data_raw) > 0:
...
def _cotizacion_ambito_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/cotizacion_ambito/CotizacionAmbitoTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("DOLAR_Y_MONEDAS_TEST_COTIZACION_AMBITO_ENTID"))

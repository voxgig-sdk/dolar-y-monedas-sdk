...
class TestBolsaDirect:

    def test_should_direct_load_bolsa(self):
        setup = _bolsa_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-bolsa", "live" if setup["live"] else "unit")
        if _skip:
...
def _bolsa_direct_setup(mockres):

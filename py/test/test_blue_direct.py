...
class TestBlueDirect:

    def test_should_direct_load_blue(self):
        setup = _blue_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-blue", "live" if setup["live"] else "unit")
        if _skip:
...
def _blue_direct_setup(mockres):

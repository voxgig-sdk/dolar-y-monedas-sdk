...
class TestCriptoDirect:

    def test_should_direct_load_cripto(self):
        setup = _cripto_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-cripto", "live" if setup["live"] else "unit")
        if _skip:
...
def _cripto_direct_setup(mockres):

...
class TestDolareDirect:

    def test_should_direct_list_dolare(self):
        setup = _dolare_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-dolare", "live" if setup["live"] else "unit")
        if _skip:
...
def _dolare_direct_setup(mockres):

...
class DolarYMonedasTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            DolarYMonedasTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = DolarYMonedasTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = DolarYMonedasTestRunner.getenv("DOLAR_Y_MONEDAS_TEST_LIVE")
        override = DolarYMonedasTestRunner.getenv("DOLAR_Y_MONEDAS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = DolarYMonedasTestRunner.getenv(key)
        ...
        explain = DolarYMonedasTestRunner.getenv("DOLAR_Y_MONEDAS_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["DOLAR_Y_MONEDAS_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if DolarYMonedasTestRunner._test_control is not None:
            return DolarYMonedasTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                DolarYMonedasTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            DolarYMonedasTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = DolarYMonedasTestRunner.load_test_control()
...
def load_env_local():
    DolarYMonedasTestRunner.load_env_local()


def env_override(m):
    return DolarYMonedasTestRunner.env_override(m)


def entity_data(v):
    return DolarYMonedasTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return DolarYMonedasTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return DolarYMonedasTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return DolarYMonedasTestRunner.load_test_control()


def live_client_options():
    return DolarYMonedasTestRunner.live_client_options()


def live_delay_ms():
    return DolarYMonedasTestRunner.live_delay_ms()

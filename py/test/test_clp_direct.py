...
class TestClpDirect:
    ...
    def test_should_direct_load_clp(self):
        setup = _clp_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-clp", "live" if setup["live"] else "unit")
        if _skip:

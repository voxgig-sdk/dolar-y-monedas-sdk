...
class TestOficialDirect:

    def test_should_direct_load_oficial(self):
        setup = _oficial_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-oficial", "live" if setup["live"] else "unit")
        if _skip:
...
def _oficial_direct_setup(mockres):

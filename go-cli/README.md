...
boru-driven command-line client **and** interactive REPL for the DolarYMonedas
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/dolar-y-monedas-cli)
make build

# 2. See usage (words, entities, env vars)
./dolar-y-monedas-cli --help

# 3. Provide credentials once, via the environment
export DOLAR_Y_MONEDAS_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./dolar-y-monedas-cli load 1 blue            # {id:1} shorthand
./dolar-y-monedas-cli load '{id:1}' blue       # explicit match map

# 5. Override the API base URL for a single call
DOLAR_Y_MONEDAS_BASE=https://api.example.com ./dolar-y-monedas-cli load 1 blue

# 6. No arguments -> interactive REPL
./dolar-y-monedas-cli
dolar-y-monedas> load 1 blue
dolar-y-monedas> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/dolar-y-monedas-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/dolar-y-monedas-cli load 1 blue
   ```
...
```sh
./dolar-y-monedas-cli load 1 blue          # scalar shorthand for {id:1}
./dolar-y-monedas-cli load '{id:1}' blue     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `dolar-y-monedas>`). Each line is
evaluated as its own boru expression:

```text
$ ./dolar-y-monedas-cli
dolar-y-monedas> load 1 blue
dolar-y-monedas> /help
dolar-y-monedas> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `blue`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/dolar-y-monedas-cli`. |
...
(`./dolar-y-monedas-cli <expr>`) and interactively (the REPL), and expressions compose the

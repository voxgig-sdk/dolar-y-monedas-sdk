...
local ClpEntity = {}
ClpEntity.__index = ClpEntity


function ClpEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ClpEntity)
  self._name = "clp"
  self._client = client
...
function ClpEntity:get_name()
  return self._name
end


function ClpEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ClpEntity.new(self._client, opts)
end
...
function ClpEntity:mark_deleted()
  self._deleted = true
end


function ClpEntity:deleted()
  return true == self._deleted
end


function ClpEntity:data_set(args)
  if args ~= nil then
...
function ClpEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ClpEntity:match_set(args)
  if args ~= nil then
...
function ClpEntity:stream(action, args, callopts)
...
function ClpEntity:_run_op(ctx, post_done)
  local utility = self._utility

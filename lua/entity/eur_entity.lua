...
local EurEntity = {}
EurEntity.__index = EurEntity


function EurEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EurEntity)
  self._name = "eur"
  self._client = client
...
function EurEntity:get_name()
  return self._name
end


function EurEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EurEntity.new(self._client, opts)
end
...
function EurEntity:mark_deleted()
  self._deleted = true
end


function EurEntity:deleted()
  return true == self._deleted
end


function EurEntity:data_set(args)
  if args ~= nil then
...
function EurEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EurEntity:match_set(args)
  if args ~= nil then
...
function EurEntity:stream(action, args, callopts)
...
---@param reqmatch EurLoadMatch
---@param ctrl? table
---@return Eur
---@return string? err
function EurEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function EurEntity:_run_op(ctx, post_done)
  local utility = self._utility

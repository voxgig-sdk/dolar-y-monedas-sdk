...
local BlueEntity = {}
BlueEntity.__index = BlueEntity


function BlueEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BlueEntity)
  self._name = "blue"
  self._client = client
...
function BlueEntity:get_name()
  return self._name
end


function BlueEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BlueEntity.new(self._client, opts)
end
...
function BlueEntity:mark_deleted()
  self._deleted = true
end


function BlueEntity:deleted()
  return true == self._deleted
end


function BlueEntity:data_set(args)
  if args ~= nil then
...
function BlueEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BlueEntity:match_set(args)
  if args ~= nil then
...
function BlueEntity:stream(action, args, callopts)
...
function BlueEntity:_run_op(ctx, post_done)
  local utility = self._utility

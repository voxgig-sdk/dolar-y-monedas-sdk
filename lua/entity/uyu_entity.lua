...
local UyuEntity = {}
UyuEntity.__index = UyuEntity


function UyuEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, UyuEntity)
  self._name = "uyu"
  self._client = client
...
function UyuEntity:get_name()
  return self._name
end


function UyuEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return UyuEntity.new(self._client, opts)
end
...
function UyuEntity:mark_deleted()
  self._deleted = true
end


function UyuEntity:deleted()
  return true == self._deleted
end


function UyuEntity:data_set(args)
  if args ~= nil then
...
function UyuEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function UyuEntity:match_set(args)
  if args ~= nil then
...
function UyuEntity:stream(action, args, callopts)
...
function UyuEntity:_run_op(ctx, post_done)
  local utility = self._utility

...
local CriptoEntity = {}
CriptoEntity.__index = CriptoEntity


function CriptoEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CriptoEntity)
  self._name = "cripto"
  self._client = client
...
function CriptoEntity:get_name()
  return self._name
end


function CriptoEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CriptoEntity.new(self._client, opts)
end
...
function CriptoEntity:mark_deleted()
  self._deleted = true
end


function CriptoEntity:deleted()
  return true == self._deleted
end


function CriptoEntity:data_set(args)
  if args ~= nil then
...
function CriptoEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CriptoEntity:match_set(args)
  if args ~= nil then
...
function CriptoEntity:match_get()
...
function CriptoEntity:stream(action, args, callopts)
...
function CriptoEntity:_run_op(ctx, post_done)
  local utility = self._utility

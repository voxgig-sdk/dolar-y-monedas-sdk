...
local BrlEntity = {}
BrlEntity.__index = BrlEntity


function BrlEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BrlEntity)
  self._name = "brl"
  self._client = client
...
function BrlEntity:get_name()
  return self._name
end


function BrlEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BrlEntity.new(self._client, opts)
end
...
function BrlEntity:mark_deleted()
  self._deleted = true
end


function BrlEntity:deleted()
  return true == self._deleted
end


function BrlEntity:data_set(args)
  if args ~= nil then
...
function BrlEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BrlEntity:match_set(args)
  if args ~= nil then
...
function BrlEntity:stream(action, args, callopts)
...
---@param reqmatch BrlLoadMatch
---@param ctrl? table
---@return Brl
---@return string? err
function BrlEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function BrlEntity:_run_op(ctx, post_done)
  local utility = self._utility

...
The Lua SDK for the DolarYMonedas API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Blue(nil):load({ id = "example_id" }, nil)
...
local result, err = client:DolarYMonedas(nil):load(
  { id = "test01" }, nil
)

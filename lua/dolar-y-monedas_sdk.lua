...
-- by passing it.
function DolarYMonedasSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function DolarYMonedasSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "DolarYMonedasSDK: graphql: " .. msg
    res.graphql = errors
  end

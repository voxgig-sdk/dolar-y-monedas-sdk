...
class DolarYMonedasSDK
    ...
    if feature_opts
      ...
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = DolarYMonedasHelpers.to_map(feature_opts[fname])
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = DolarYMonedasError.new(
        "graphql_error", "DolarYMonedasSDK: graphql: #{msg}")
      res["graphql"] = errors
    end

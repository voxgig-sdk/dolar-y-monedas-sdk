...
class CotizacionAmbitoEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.CotizacionAmbito(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.CotizacionAmbito(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = cotizacion_ambito_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "cotizacion_ambito." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set DOLAR_Y_MONEDAS_TEST_COTIZACION_AMBITO_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    cotizacion_ambito_ref01_data = nil
    if cotizacion_ambito_ref01_data_raw.length > 0
...
def cotizacion_ambito_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "cotizacion_ambito", "CotizacionAmbitoTestData.json")
  ...
  entid_env_raw = ENV["DOLAR_Y_MONEDAS_TEST_COTIZACION_AMBITO_ENTID"]

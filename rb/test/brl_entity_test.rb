...
class BrlEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = brl_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "brl." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    brl_ref01_data = nil
    if brl_ref01_data_raw.length > 0
...
def brl_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "brl", "BrlTestData.json")

...
class CriptoEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = cripto_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "cripto." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set DOLARYMONEDAS_TEST_CRIPTO_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    cripto_ref01_data = nil
    if cripto_ref01_data_raw.length > 0
...
def cripto_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "cripto", "CriptoTestData.json")
  ...
  entid_env_raw = ENV["DOLARYMONEDAS_TEST_CRIPTO_ENTID"]

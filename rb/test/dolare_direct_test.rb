...
class DolareDirectTest < Minitest::Test
  def test_direct_list_dolare
    setup = dolare_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-dolare", setup[:live] ? "live" : "unit")
    if _should_skip
...
def dolare_direct_setup(mockres)

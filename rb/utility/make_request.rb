...
module DolarYMonedasUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = DolarYMonedasResponse.new({})
    ...
    if fetch_err
      ...
      response = DolarYMonedasResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = DolarYMonedasResponse.new(fetched)
    else

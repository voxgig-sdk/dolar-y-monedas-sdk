...
class CotizacionAmbitoEntity
  ...
  def make
    opts = @_entopts.dup
    CotizacionAmbitoEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.CotizacionAmbito.load works with no args).
  ...
  #
  # @param reqmatch [CotizacionAmbitoListMatch, Hash, nil] match filter (any subset of
  #   CotizacionAmbito fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control

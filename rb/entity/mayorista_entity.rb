...
class MayoristaEntity
  ...
  def make
    opts = @_entopts.dup
    MayoristaEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = DolarYMonedasHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Mayorista, Hash] the current Mayorista data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = DolarYMonedasHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Mayorista fields)
  def match_get
  ...
  #
  # @param reqmatch [MayoristaLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Mayorista.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Mayorista, Hash] the loaded Mayorista; raises DolarYMonedasError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = DolarYMonedasHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end

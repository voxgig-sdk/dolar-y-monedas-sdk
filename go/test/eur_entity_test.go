...
func TestEurEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		eurRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.eur")))
...
func eurBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("DOLAR_Y_MONEDAS_TEST_EUR_ENTID")
	...
	idmapResolved := core.ToMapAny(env["DOLAR_Y_MONEDAS_TEST_EUR_ENTID"])

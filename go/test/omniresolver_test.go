...
import (
	...
	omni "github.com/voxgig-sdk/dolar-y-monedas-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.DolarYMonedasError); is {
				out := map[string]any{
					"name":    "DolarYMonedasError",
...
func omniCtx(arg any, client *sdk.DolarYMonedasSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.DolarYMonedasSDK); is {
			client = live

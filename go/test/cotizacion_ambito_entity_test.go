...
func TestCotizacionAmbitoEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set DOLARYMONEDAS_TEST_COTIZACION_AMBITO_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func cotizacion_ambitoBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("DOLARYMONEDAS_TEST_COTIZACION_AMBITO_ENTID")
	...
	idmapResolved := core.ToMapAny(env["DOLARYMONEDAS_TEST_COTIZACION_AMBITO_ENTID"])

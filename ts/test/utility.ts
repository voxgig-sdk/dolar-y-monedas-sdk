...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.DOLARYMONEDAS_TEST_LIVE ||
    'TRUE' === process.env.DOLARYMONEDAS_TEST_OVERRIDE
  ) {
  ...
  m.DOLARYMONEDAS_TEST_EXPLAIN = process.env.DOLARYMONEDAS_TEST_EXPLAIN || m.DOLARYMONEDAS_TEST_EXPLAIN

...
describe('CotizacionAmbitoDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new DolarYMonedasSDK({
  ...
  test('direct-load-cotizacion_ambito', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-cotizacion_ambito', setup.live)) return
  ...
  test('direct-list-cotizacion_ambito', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-cotizacion_ambito', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['DOLAR_Y_MONEDAS_TEST_COTIZACION_AMBITO_ENTID']

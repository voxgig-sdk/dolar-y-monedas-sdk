...
describe('CotizacioneDirect', async () => {
  ...
  test('direct-list-cotizacione', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-cotizacione', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['DOLAR_Y_MONEDAS_TEST_COTIZACIONE_ENTID']

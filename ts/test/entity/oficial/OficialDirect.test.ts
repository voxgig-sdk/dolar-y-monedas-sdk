...
describe('OficialDirect', async () => {
  ...
  test('direct-load-oficial', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-oficial', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['DOLAR_Y_MONEDAS_TEST_OFICIAL_ENTID']

...
describe('TarjetaDirect', async () => {
  ...
  test('direct-load-tarjeta', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-tarjeta', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['DOLAR_Y_MONEDAS_TEST_TARJETA_ENTID']

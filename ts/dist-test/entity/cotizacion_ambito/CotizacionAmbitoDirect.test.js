...
(0, node_test_1.describe)('CotizacionAmbitoDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.DolarYMonedasSDK({
    ...
    (0, node_test_1.test)('direct-load-cotizacion_ambito', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-cotizacion_ambito', setup.live))
            return;
    ...
    (0, node_test_1.test)('direct-list-cotizacion_ambito', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-cotizacion_ambito', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.DOLAR_Y_MONEDAS_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.DolarYMonedasSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['DOLAR_Y_MONEDAS_TEST_COTIZACION_AMBITO_ENTID'];
    ...
    const client = new __1.DolarYMonedasSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=CotizacionAmbitoDirect.test.js.map
...
// DolarYMonedas SDK retry feature
...
class DolarYMonedasRetryFeature extends DolarYMonedasBaseFeature
{
    ...
    public function init(DolarYMonedasContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (DolarYMonedasContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(DolarYMonedasContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(DolarYMonedasContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
